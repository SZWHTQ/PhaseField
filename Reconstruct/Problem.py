...
class IsotropicPlasticProblem(DisplacementProblem):
    def __init__(
        ...
        self.velocity = dfx.fem.Function(self.V, name="Velocity")
        self.velocity_old = dfx.fem.Function(self.V)
        self.velocity_inc = dfx.fem.Function(self.V, name="Velocity increment")

        self.acceleration = dfx.fem.Function(self.V, name="Acceleration")
        self.acceleration_old = dfx.fem.Function(self.V)
        self.acceleration_inc = dfx.fem.Function(self.V, name="Acceleration increment")
        ...
        self.F += (
            ufl.inner(
                con.getStrain(self.v),
                con.asTensor3x3(con.stress_vector),
            )
            * self.dx
        )
        ...
        self._stress_vector_vis = dfx.fem.Function(self._W_vis, name="Stress")
    ...
    def solve(self):
        ...
        deviatoric_stress = ufl.dev(con.asTensor3x3(con.stress_vector))
        Util.localProject(
            ufl.sqrt(
                1.5
                * ufl.inner(
...
class DuctileFractureSubProblem(PlaneStrainProblem):
    def __init__(
        ...
        self.iteration_correction = dfx.fem.Function(
            self.S, name="Iteration correction"
        )  # Iteration correction for crack phase in Newton-Raphson method
...
class DuctileFractureProblem(PlaneStrainProblem):
    ...
    def solve(self):
        ...
        con.elastic_strain_vector_old.x.array[:] = con.elastic_strain_vector.x.array[:]
        con.equivalent_plastic_strain_old.x.array[:] = (
            con.equivalent_plastic_strain.x.array[:]
        )
